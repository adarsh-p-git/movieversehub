...
function Searchmovie() {
  ...
  const [randmov,setRandMov]=useState();

const randomWordfn=()=>
{
  const Words=["avenger","mission","mars","space","star","spider","spy","adventure","verse"]
 ...
 return Words[Math.floor(Math.random()*Words.length)]
  ...
  const randomMovieSearch=async()=>
  {
    try{
      let res1;
      
      res1=await axios.get(`https://www.omdbapi.com/?s=${randomWordfn()}&apikey=778c217`)
      setRandMov(res1.data)
      console.log("random",res1.data)
    }
    catch(error)
    {
      console.log("Error in Random movie:",error)
    }
  }
  useEffect(()=>
  { randomWordfn()
    ...
    <div>
      <div className="container1 container-lg   p-2 align-items-center">
...

        {
          ...
          </div>) : (<div className='moviedisplay d-flex flex-wrap justify-content-center'>
            {
              randmov && randmov.Search?.map((item)=>
              (
                <div className='m-2 p-1' key={item.imdbID}>
                  <Card style={{width:'18rem'}}>
